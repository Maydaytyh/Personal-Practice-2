<template class="retriveUserAdmin">
    <el-row :gutter="20">
    <el-col :span="4">
...
export default {
  name: 'retriveUserAdmin',
  template: '<retriveUserAdmin>/',
  props: ['currentPage'],
  currentPageNum: 0,
  urlDef: '',
  created () {
    Axios({
      method: 'get',
      url: 'http://192.168.241.11:8086/user/q/size?name&description&province&city'
    }).then((response) => {
    ...
    Axios({
      method: 'get',
      url: 'http://192.168.241.11:8086/user/q/city'
    }).then((response) => {
      this.cities = response.data
    })
    // 初始化获取10个用户数据
    this.urlDef = 'http://192.168.241.11:8086/user/q/user?currentPage=1&pageSize=10&&description&province&city'
    this.getData()
  },
  // 父子组件数据状态同步
  watch: {
  ...
  methods: {
    ...
    searchUser: function () {
      if (this.inputName !== '' || this.inputDescription !== '' || this.inputProvince !== '' || this.inputCity !== '') {
        this.urlDef = 'http://192.168.241.11:8086/user/q/user?currentPage=' + this.currentPageNum + '&pageSize=10&name=' + this.inputName + '&description=' + this.inputDescription + '&province=' + this.inputProvince + '&city=' + this.inputCity
        Axios({
          method: 'get',
          url: 'http://192.168.241.11:8086/user/q/size?name=' + this.inputName + '&description=' + this.inputDescription + '&province=' + this.inputProvince + '&city=' + this.inputCity
        }).then((response) => {
          this.retriveSize = response.data
        })
        this.getData()
