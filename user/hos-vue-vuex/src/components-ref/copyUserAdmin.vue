<template>
  <div class="copyUserAdmin">
      <el-button type="info" size="small" @click="clickUpdate()">复制用户</el-button>

      <el-dialog title="复制用户" @click="closeFunc()" :visible.sync="dialogFormVisible">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      postData: function () {
        if(this.userForm1.name!==''&&this.userForm1.description!==''&&this.userForm1.password!==''){
      ...
      clickUpdate: function () {
        ...
        Axios({
          method: 'get',
          url: 'http://192.168.241.11:8086/user/q/city?province=' + this.userForm1.province
        }).then((response) => {
          this.citiesData = response.data
        })
        Axios.get('http://192.168.241.11:8086/user/q/net?city=' + this.userForm1.city)
        .then((response) => {
