  ...
  <div class="updateUserSuper">
      ...
      <el-dialog title="修改用户" :visible.sync="dialogFormVisible" @close="closeFunc()">
        <el-form :model="userForm" :rules="rules">
          ...
          <el-form-item label="用户说明" :label-width="formLabelWidth" porp="description" required>
  ...
  export default {
    ...
    watch: {
      finalData: function () {
        ...
        this.userForm.groups=this.userForm.groups.map(function(x,i,arr){
          if(i!==arr.length-1&&x.indexOf("，")>0){ return x.substring(0,x.indexOf("，"))}
          else{
            return x
          }
        })
        Axios({
          method: 'get',
          url: Ip.bikeDev+'user/q/city?province=' + this.userForm.province
        }).then((response) => {
          this.citiesData = response.data
        })
        Axios({
          method: 'get',
          url: Ip.bikeDev+'user/q/net?city=' + this.userForm.city
        }).then((response) => {
    ...
    methods: {
      ...
      postData: function () {
        if(this.userForm.name!==''&&this.userForm.enable!==''&&this.userForm.password!==''&&this.userForm.groups.length>0){
          this.dialogFormVisible = false
          Axios({
            method: 'post',
            url: Ip.bikeDev+'user?update',
            data: this.userForm
          }).then((response) => {
            // this.clearData()
          })
      ...
      clickUpdate: function () {
        ...
        Axios.get(Ip.bikeDev+'user/q/net?city=' + this.userForm.city)
        .then((response) => {
          this.netsData = response.data
        })
      },
