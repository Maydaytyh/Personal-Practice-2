<template>
  <div class="createUserSuper">
      <el-button type="info" v-on:click="showForm()">新增</el-button>

      <el-dialog title="新增用户" :visible.sync="operation.dialogFormVisible" class="createBody">
...
<script>
  ...
  export default {
    name: 'createUserSuper',
    template: '<createUserSuper/>',
    citiesUrl: '',
    netsUrl: '',
    created () {
      this.getData()
    ...
    methods: {
      ...
      // 提交表单数据
      postData: function () {
        this.operation.dialogFormVisible = false
        if (this.userForm.password === this.userForm.confirmPassword&&this.userForm.groups.length>0) {
          Axios({
            method: 'post',
      ...
      showForm:function(){
        this.operation.dialogFormVisible = true;
      }
    },
    data () {
      return {
